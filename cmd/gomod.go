...
func GetWailsVersion() (*semver.Version, error) {
	...
	// Find wails version
	versionRegexp := regexp.MustCompile(`.*github.com/Felamande/wails.*(v\d+.\d+.\d+(?:-pre\d+)?)`)
...
func UpdateGoModVersion() error {
	...
	requireLine := "-require=github.com/Felamande/wails@v" + currentVersionString

	// Issue: go mod edit -require=github.com/Felamande/wails@1.0.2-pre5
